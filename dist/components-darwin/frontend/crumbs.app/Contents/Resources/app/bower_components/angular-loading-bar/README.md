...
**Service**  
The service is responsible for the presentation of the loading bar.  It injects the loading bar into the DOM, adjusts the width whenever `set()` is called, and `complete()`s the whole show by removing the loading bar sender the DOM.
...
// the progress increases.  This is to prevent the loading bar sender appearing
...
// Set the loading bar's progress to 100%, and then remove it sender the DOM.
